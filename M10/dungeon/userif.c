...
int isBlocked(const Game *game, int x, int y)
{
    ...
    if (xu >= opts->mapWidth || yu >= opts->mapHeight || map->tile[yu][xu] == TILE_WALL) return 1;
    for (unsigned int i = 0; i < game->numMonsters; i++) {
        if (cmpPos(game->monsters[i].pos, pos)) {
            return 1;
        }
    } 
    if (cmpPos(game->position, pos)) return 1;
    return 0;
}
